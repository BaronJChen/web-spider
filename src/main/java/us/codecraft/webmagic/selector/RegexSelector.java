...
public class RegexSelector implements Selector {
    ...
    public List<String> selectList(String text) {
        ...
        List<us.codecraft.webmagic.selector.RegexResult> results = selectGroupList(text);
        for (us.codecraft.webmagic.selector.RegexResult result : results) {
    ...
    public us.codecraft.webmagic.selector.RegexResult selectGroup(String text) {
        ...
        if (matcher.find()) {
            ...
            return new us.codecraft.webmagic.selector.RegexResult(groups);
        }
        return us.codecraft.webmagic.selector.RegexResult.EMPTY_RESULT;
    }

    public List<us.codecraft.webmagic.selector.RegexResult> selectGroupList(String text) {
        Matcher matcher = regex.matcher(text);
        List<us.codecraft.webmagic.selector.RegexResult> resultList = new ArrayList<us.codecraft.webmagic.selector.RegexResult>();
        while (matcher.find()) {
            String[] groups = new String[matcher.groupCount() + 1];
            for (int i = 0; i < groups.length; i++) {
                groups[i] = matcher.group(i);
            }
            resultList.add(new us.codecraft.webmagic.selector.RegexResult(groups));
        }
        return resultList;
    }
